...
void createProcess(uint64_t ip, unsigned int argc, char argv[6][21]) {
  uint32_t size = 4096;
  uint8_t priority = (compareStrings("&", argv[argc-1]) ? 3 : 1);
  if (priority == 3)
    argc--;
  ...
  sys_createProcess(ip, size, priority, argc, args);
  sys_free(args);
}
...
void executeCommand(char * buffer) {
	...
	if (compareStrings(args[0], "help")) {
		...
		createProcess((uint64_t)&p1, argNum, args);
	else if (compareStrings(args[0], "loop")) {
		createProcess((uint64_t)&loop, argNum, args);
